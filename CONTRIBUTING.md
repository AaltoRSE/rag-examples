...
1. Each example should be in a subfolder of the relevant topic.

   For example, `langchain-pdf-ingestion`-example concerns data
   loading and preprocessing, so it is under `data-ingestion`-folder.

   If there is no folder that describes the topic of your example, you can
   create a new folder.
1. Each example should contain a `README.md` that describes the example
   and gives instructions on how to run the example.
1. If the example uses a Python environment, include a `requirements.txt` or
   `environment.yml` that can be used to create the environment needed by
   the example.
1. If the code is meant to be run in a cluster environment, please add an
